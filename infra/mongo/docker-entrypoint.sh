...
_parse_config() {
	...
	if configPath="$(_mongod_hack_get_arg_val --config "$@")"; then
		...
		if grep -vEm1 '^[[:space:]]*(#|$)' "$configPath" | grep -qE '^[[:space:]]*[^=:]+[[:space:]]*='; then
			# if the first non-comment/non-blank line of the config file looks like "bar = ...", this is probably the 2.4 and older "ini-style config format"
		...
		if [ "$(head -c1 "$jsonConfigFile")" != '{' ] || [ "$(tail -c2 "$jsonConfigFile")" != '}' ]; then
			# if the file doesn't start with "{" and end with "}", it's *probably* an error ("uncaught exception: YAMLException: bar" for example), so we should print it out
